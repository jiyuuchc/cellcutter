...
def decode_one_img(masks_one_img, bboxes_one_img, scores_one_img, threshold, h, w):
    ...
    def decode_one_mask(inputs):
        ...
        if box[3] > box[1]:
            ...
            c1 = box[1]
            mask = mask[:,::-1]
        mask = tf.image.resize(mask, [r1-r0, c1-c0])
        mi = tf.where(mask[:,:,0] >= .5) + [r0,c0]
    ...
    mask_stack = tf.map_fn(
        decode_one_mask,
        [tf.cast(masks_one_img, tf.float32), bboxes_one_img],
        fn_output_signature=tf.TensorSpec((h, w), tf.bool),
    )
...
class MaskLayer(tf.keras.layers.Layer):
    ...
    def predict(self, inputs):
        decoder_out, bboxes, scores, cls = inputs
        _, h, w, _ = decoder_out['0'].get_shape()
        ...
        mask_indices, scores = tf.map_fn(
            ...
            [masks, boxes, scores],
            fn_output_signature=(tf.RaggedTensorSpec((None,3), tf.int64, 0), tf.RaggedTensorSpec((None,), tf.float32, 0)),
        )
        return mask_indices, scores
